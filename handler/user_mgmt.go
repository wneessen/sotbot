...
func UserIsRegistered(u *user.User) (string, bool) {
	if u.IsRegistered() {
		responeMsg := "You are a registered user!"
		return responeMsg, true
	}
...
// Register a new user
func RegisterUser(d *gorm.DB, u string) (string, bool, error) {
	...
	if err != nil {
		...
		return "", false, fmt.Errorf("Could not look up if user exists in DB: %v", err)
	}
	if dbUser.ID > 0 {
	...
	if err := database.CreateUser(d, validUserMatches[1]); err != nil {
		...
		return "", false, fmt.Errorf("Could not store user in DB: %v", err)
	}
...
// Unregister a user
func UnregisterUser(d *gorm.DB, u string) (string, bool, error) {
	l := log.WithFields(log.Fields{
	...
	if err := database.DeleteUser(d, &dbUser); err != nil {
		...
		return "", false, fmt.Errorf("Could not delete user in DB: %v", err)
	}
